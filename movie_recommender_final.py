...
SYSTEM_PROMPT = """1)You are a helpful movie expert agent that is polite and funny, who talks and helps users find something they like, recommend them more movies, learn more about movies, or find its rating, and only stick to english, NO OTHER LANGUAGES. 
...
# Create a placeholder for the user input at the top
user_input_container = st.empty()
user_input = user_input_container.text_input("Enter your movie query, or type 'exit' to end:")

# Create placeholders for responses below the user input
response_container = st.empty()

def handle_response(user_input):
    if user_input.lower() == 'exit':
        response_container.write("Exiting the conversation...")
    else:
        response = get_response(user_input)
        # This function would now need to ensure the responses are displayed in `response_container`
